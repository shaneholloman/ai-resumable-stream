...
export async function createResumableUIMessageStream(options: CreateResumableUIMessageStream) {
  ...
  async function startStream(
    ...
    /**
     * Create a new resumable stream in Redis.
     * resumable-stream eagerly drains the source and enqueues directly to the returned stream.
     */
    const sseStream = await context.createNewResumableStream(streamId, () =>
      convertUIMessageToSSEStream(stream),
    );
    ...
    /**
     * Convert the SSE stream back to UI message chunks for the client.
     */
    const uiStream = convertSSEToUIMessageStream(sseStream, () => unsubscribe());
